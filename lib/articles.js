...
export function renderMarkdown(md){
 const lines=md.split("\n"),out=[];let list=[],ordered=false;
 const flush=()=>{if(list.length){const Tag=ordered?"ol":"ul";out.push(<Tag key={`list-${out.length}`}>{list.map((x,i)=><li key={i}>{inline(x)}</li>)}</Tag>);list=[]}};
 ...
 for(let i=0;i<lines.length;i++){
  ...
  if(line.includes("|")&&i+1<lines.length&&divider(lines[i+1])){
   flush();const headers=cells(line),rows=[];i+=2;
   while(i<lines.length){const r=lines[i].trim();if(!r||!r.includes("|")){i--;break}rows.push(cells(r));i++}
   out.push(<div className="article-table-wrap" key={`table-${out.length}`}><div className="article-table-scroll" role="region" aria-label="Scrollable comparison table" tabIndex="0"><table className="article-table"><thead><tr>{headers.map((c,j)=><th key={j}>{inline(c)}</th>)}</tr></thead><tbody>{rows.map((row,r)=><tr key={r}>{headers.map((_,c)=><td key={c}>{inline(row[c]||"")}</td>)}</tr>)}</tbody></table></div><div className="article-table-hint" aria-hidden="true">Swipe to compare →</div></div>);
   continue;
  }
  if(line.startsWith("- ")){if(ordered)flush();ordered=false;list.push(line.slice(2));continue}
  if(/^\d+\.\s/.test(line)){if(!ordered&&list.length)flush();ordered=true;list.push(line.replace(/^\d+\.\s/,""));continue}
  flush();
  if(line.startsWith("### ")){const t=line.slice(4);out.push(<h3 id={slugify(t)} key={i}>{inline(t)}</h3>)}
  else if(line.startsWith("## ")){const t=line.slice(3);out.push(<h2 id={slugify(t)} key={i}>{inline(t)}</h2>)}
